                    ...
                    <form class="sprsearch-form {$display_box_select|escape:'html':'UTF-8'}" method="get" action="{$search_controller_url}">
						...
						<input type="hidden" name="controller" value="search">
                            <input class="spr-query" type="text" name="s"
                                   ...
                                   placeholder="{l s='Search entire store here...' d='Shop.Theme.Global'}"/>
						</div>
                        <button value="{l s='Search'}" class="spr-search-button" type="submit" name="spr_submit_search">
                            <i class="fa fa-search"></i>
							{if isset($isMobile_n) && $isMobile_n && isset($SP_MobileLayout) && $SP_MobileLayout}
								<span class="search-text-mobile">{l s='Search' d='Shop.Theme.Global'}</span>
							{/if}
                        </button>
