				...
				<div class="moduletable clearfix {$moduleclass_sfx|escape:'html':'UTF-8'}">
                    <div id="{$uniqued|escape:'html':'UTF-8'}" class="sp-countdown-sliders"					
						{foreach from=$items->params item=param key=k} 
							data-{$k} = "{$param}" 
						{/foreach}>
	                        <div class="pds-items-detail col-md-10 col-xs-9">
													...
													<div class="product-item-bg">
														<div class="row">
															...
															<div class="pb-left-column col-xs-12 col-sm-12 col-md-5 col-lg-5">
																...
																<div class="highlighted-informations quick-view-wrapper{if !$product.main_variants} no-variants{/if}" title = "{l s='Quick View' mod='spcountdownproduct'}">
															...
															<div class="pds-content product-item product-detail col-xs-12 col-sm-12 col-md-7 col-lg-7">
																...
																<div class="product-description">												
																	...
																	{block name='product_name'}
																		<div class="pds-title">
																			<h4><a href="{$product.link}" {$product._target nofilter}>{$product.title nofilter}</a></h4>
																		</div>
																	{/block}
																	{/if}  
																	...
																	{if $items->params.display_description}
																		{block name='product_description_short'}
																			<div class="product-description" itemprop="description">{$product.description_short|truncate:150 nofilter}</div>
																		{/block}
																	{/if}
																	...
																	{if $items->params.display_addtocart}
																		{if $product.availability == 'available'}
																			<a href="#" class="ajax-add-to-cart product-btn cart-button" data-id-product="{$product.id_product}" data-minimal-quantity="1" title = "{l s='Add to Cart' mod='spcountdownproduct'}">
																				<span class="text">{l s='Add to Cart' mod='spcountdownproduct'}</span>
																			</a>
																		...
																		{else}
																			<span class="no-product cart-button" data-id-product="{$product.id_product}" data-minimal-quantity="1" title = "{l s='Add to Cart' mod='spcountdownproduct'}">
																				<span class="text">{l s='Add to Cart' mod='spcountdownproduct'}</span>
																			</a>
																		{/if}
																	{/if}
							...
							<div class="thumb-image col-md-2 col-xs-3">
								<div class="pds-items">
									{foreach from=$_list  item=product key =k}
