    ...
    {foreach from=$list item=items}
        ...
        <div class="moduletable  {$moduleclass_sfx|escape:'html':'UTF-8'}">
            ...
            {if isset($_list) && $_list}
                ...
                {$_list = $items->products}
                {if isset($_list) && $_list}
	                <div id="{$tag_id|escape:'html':'UTF-8'}" data-tag_id="{$tag_id|escape:'html':'UTF-8'}" 
	                 	class="sp-extraslider sp-preload {$cls_btn_page|escape:'html':'UTF-8'} {$class_respl|escape:'html':'UTF-8'}
						 ...
						 data-effect="{$items->params.effect|escape:'html':'UTF-8'}">
	                        ...
	                        {foreach $_list as $product}
	                            ...
	                            {if $count % $nb_rows == 1 || $nb_rows == 1}
	                                <div class="item">
	                            {/if}
								<article class="product-miniature js-product-miniature {(($count % $nb_rows == 1) ? 'product-biger' : '')}" data-id-product="{$product.id_product}" data-id-product-attribute="{$product.id_product_attribute}" itemscope itemtype="http://schema.org/Product">		
												...
												<div class="product-image">
													{block name='product_thumbnail'}
														<a href="{$product.link|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
															{if $count % $nb_rows == 1}
																{assign var="src" value={$link->getImageLink($product.link_rewrite, $product.id_image, 'bigger_img_default')|escape:'html':'UTF-8'}}
															{else}
															...
															{if $count % $nb_rows !== 1}
																{assign var="imageSize" value=($items->params.image_size != 'none') ? $items->params.image_size : 'layout6_default'}
																{if $SP_secondimg}
											...
											<div class="right-block">
												<div class="product-info">
													{if isset($product.category)}
														<div class="cate-default-name">{$product.category}</div>
													{/if}
														...
														<div class="image-thumnail">
															<ul>
																{if !empty($product.images)}
																	{assign var="check_break" value="1"}
																	{foreach from=$product.images item=image}
																	  <li class="item col-xs-4">
																		<img
																		   class="thumb-image  {if $image.id_image == $product.cover.id_image || $check_break == 1} thumb-selected {/if}"
																		  data-image-medium-src="{$image.bySize.medium_default.url}"
																		  data-image-large-src="{$image.bySize.bigger_img_default.url}"
																		  src="{$image.bySize.layout6_default.url}" alt="{$image.legend}" title="{$image.legend}" itemprop="image">
																	  </li>
																	  {if $check_break == 3}{break}{/if}
																	  {assign var="check_break" value=$check_break+1}
																	{/foreach}
																{/if}
															</ul>
														</div>
