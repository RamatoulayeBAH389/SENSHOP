				...
				<div class="navbar-header">
					<button type="button" id="show-vermegamenu" data-toggle="collapse" data-target=".sp-vermegamenu" class="navbar-toggle">
						{if isset($isMobile_n) && $isMobile_n && isset($SP_MobileLayout) && $SP_MobileLayout}
							<i class="fa fa-bars"></i> <span>{$item.title}</span>
						{else}
			...
			$(document).ready(function() {
				...
				if(wd_width >= 1400)
					...
					var limit = {/literal}{$spverticalmenu_params.limit3}{literal} -1 ;
                                if($('#sp-vermegamenu .item-1').length > limit){
                                    $('#sp-vermegamenu > ul').append('<div class="more-wrap"><i class="fa fa-plus-circle"></i><span class="more-view">{/literal}{l s='More Categories' d='Shop.Theme.Global'}{literal}</span></div>');
                                }
				...
				$('#sp-vermegamenu .more-wrap').click(function(){
					if($(this).hasClass('open')){
						...
						$('.more-wrap').html('<i class="fa fa-plus-circle"></i><span class="more-view">{/literal}{l s='More Categories' d='Shop.Theme.Global'}{literal}</span>');
					}else{
						$('#sp-vermegamenu .item-1').each(function(i){
							if(i>limit){
								$(this).slideDown(200);
						...
						$(this).addClass('open');
						$('.more-wrap').html('<i class="fa fa-minus-circle"></i><span class="more-view">{/literal}{l s='Close Menu' d='Shop.Theme.Global'}{literal}</span>');
					}
				});
