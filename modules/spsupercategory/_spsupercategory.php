...
class SpSuperCategory extends Module
{
	...
	public function hookdisplayHeader()
	{
		...
		$this->context->controller->registerStylesheet('modules-animate', 'modules/'.$this->name.'/views/css/animate.css', ['media' => 'all', 'priority' => 155]);
		$this->context->controller->registerStylesheet('modules-owl.carousel', 'modules/'.$this->name.'/views/css/owl.carousel.css', ['media' => 'all', 'priority' => 155]);
		...
		{
			$this->context->controller->registerJavascript('modules-owl.carousel', 'modules/'.$this->name.'/views/js/owl.carousel.js', ['position' => 'bottom', 'priority' => 155]);
	...
	public function hookDisplayCustom($hook = 'displayCustom')
	{
		...
		if (!$this->isCached ('default.tpl', $smarty_cache_id))
		{
			...
			$smarty->assign (array(
				'list' => $list,
				'spbase_dir'            => _PS_BASE_URL_.__PS_BASE_URI__,
				'id_lang' => $this->context->language->id
			));
		}
		...
		public function hookdisplaySuperCategory4()
	{
		...
		if (!$this->isCached ('default_2.tpl', $smarty_cache_id))
		{
		...
		return $this->display (__FILE__, 'default_2.tpl', $smarty_cache_id);
	}
