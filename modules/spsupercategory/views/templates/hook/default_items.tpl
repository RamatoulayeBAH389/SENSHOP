  ...
  <div class="item-image">
    {block name='product_thumbnail'}	
      <a href="{$product.link}" {$product._target nofilter} class="thumbnail product-thumbnail">
        <img
          src = "{$product['cover']['bySize'][$items_params.image_size]['url']}"
	...
	{if $items_params.display_name == 1}
      {block name='product_name'}
        <h1 class="h3 item-title" itemprop="name"><a href="{$product.link}" {$product._target nofilter} >{$product.title nofilter}</a></h1>
      {/block}
	{/if}  
