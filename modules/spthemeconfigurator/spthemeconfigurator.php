...
{	
    ...
    public function hookHeader($params)
    {
		...
		if (!defined ('OWL_CAROUSEL')){
			$this->context->controller->registerJavascript('owl.carousel.js', '/assets/sp-js/owl.carousel.js', ['position' => 'bottom', 'priority' => 1]);
		...
		$this->context->controller->registerStylesheet('modules-animate', 'modules/'.$this->name.'/views/css/front/animate.css', ['media' => 'all', 'priority' => 155]);
		$this->context->controller->registerStylesheet('modules-owl.carousel', 'modules/'.$this->name.'/views/css/front/owl.carousel.css', ['media' => 'all', 'priority' => 155]);
		// Load Cpanel Config
		 if($showCpanel){ 
	...
	public function hookDisplaySecondImage($params) {
			...
			$this->smarty->assign(array(
				...
				'images' => $_images,
				'imageSize' => isset($params['imageSize']) ? $params['imageSize'] : 'home_default'
			));
		return $this->display(__FILE__, 'displaySecondImage.tpl');
	}
	public function hookDisplayAllImage($params) {
		...
		return $this->display(__FILE__, 'displayAllImage.tpl');
	}
	/* ------------------------------------------------------------- */
