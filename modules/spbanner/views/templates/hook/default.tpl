...
{if isset($list) && !empty($list)}
    {foreach from=$list item=item}
        ...
        <div class="{$moduleclass_sfx|escape:'html':'UTF-8'} {$effect_sfx|escape:'html':'UTF-8'}  spbanner">
            ...
            {/if}
			{if isset($item.image)}
			<a href="{if $item.banner_link}{$item.banner_link|escape:'htmlall':'UTF-8'}{else}{if isset($force_ssl) && $force_ssl}{$urls.base_dir_ssl}{else}{$urls.base_dir}{/if}{/if}" title="{$item.title_module|escape:'htmlall':'UTF-8'}">			
