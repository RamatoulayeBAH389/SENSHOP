...
<form id="spsco_form" autocomplete="on">
    {if isset($SPSCO_FIELDS[$SPSCO_GLOBALS->object->customer])}
        <h4 class="spsco-title spsco-title_one cf">
            <i class="fa fa-user "></i>
            {l s='Your Details' mod='spstepcheckout'}
            {if !$customer.is_logged}
                <button type="button" class="btn btn-info btn-sm  pull-right" data-toggle="modal" data-target="#spsco_login_modal">
                    {l s='Log in' mod='spstepcheckout'}
                </button>
			{else}
				 <button  data-link="{$urls.actions.logout}" title="{l s='Log me out' mod='spstepcheckout'}" class="btn btn-light btn-sm pull-right btn-logout">
					{l s='Log out' mod='spstepcheckout'}
				</button>
            {/if}
        </h4>
